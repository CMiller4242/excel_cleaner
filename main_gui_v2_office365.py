...
if __name__ == "__main__":
    ...
    def on_login_success(session_token):
        """Called when user successfully logs in"""
        global global_session_token, global_user_email
        
        auth_manager = AuthManager()
        is_valid, user_email = auth_manager.verify_session(session_token)
    ...
    login_window = LoginWindow(on_success_callback=on_login_success)
    session_token = login_window.run()
    
    # If login was successful (session_token returned), start main app
    if session_token or global_session_token:
        token = session_token if session_token else global_session_token
        
        auth_manager = AuthManager()
        is_valid, user_email = auth_manager.verify_session(token)
        
        if is_valid:
            logging.info(f"Starting main app for user: {user_email}")
            
            # Start the Excel Tool
            root = tk.Tk()
        ...
        logging.info("Login window closed without authentication")